...
describe("GET /auth/callback", () => {
  it("exchanges a PKCE code and forwards the verified merchant to /onboarding", async () => {
    const res = await callGet("https://infinityafrica.net/auth/callback?code=abc123&next=/onboarding");

    expect(exchangeCodeForSession).toHaveBeenCalledWith("abc123");
    expect(res.status).toBe(307);
    expect(res.headers.get("location")).toBe("https://infinityafrica.net/onboarding");
  });

  it("verifies a token_hash / type=signup link (cross-device style)", async () => {
    const res = await callGet(
      "https://infinityafrica.net/auth/callback?token_hash=xyz&type=signup",
    );
  ...
  it("sends a failed exchange to /merchant/login with a plain-language notice, never a 400", async () => {
    ...
    const res = await callGet("https://infinityafrica.net/auth/callback?code=stale");
  ...
  it("treats a link opened with no token as 'verified — please sign in', not an error page", async () => {
    const res = await callGet("https://infinityafrica.net/auth/callback");
  ...
  it("routes an error_description straight to login without touching Supabase", async () => {
    const res = await callGet(
      "https://infinityafrica.net/auth/callback?error_description=Email+link+is+invalid+or+has+expired",
    );
  ...
  it("ignores an off-site `next` (open-redirect guard) and falls back to /onboarding", async () => {
    const res = await callGet(
      "https://infinityafrica.net/auth/callback?code=ok&next=https://evil.example.com",
    );
  ...
  it("does not re-run the exchange when the same request is handled once (single consume)", async () => {
    await callGet("https://infinityafrica.net/auth/callback?code=abc123");
