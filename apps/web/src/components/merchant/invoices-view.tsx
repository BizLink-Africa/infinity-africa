...
import { createInvoice, generateInvoicePaymentLink, listInvoices } from "@/lib/portal/api";
...
export function InvoicesView() {
  ...
  async function handleCopyPayNowLink(invoice: Invoice) {
    setCopyError("");
    setCopyingId(invoice.id);
    try {
      const link = await generateInvoicePaymentLink(invoice.id);
      await navigator.clipboard.writeText(link.public_url);
      setCopiedId(invoice.id);
      setTimeout(() => setCopiedId((current) => (current === invoice.id ? null : current)), 2000);
    } catch (err) {
      setCopyError(err instanceof Error ? err.message : "Couldn't generate a Pay Now link for this invoice.");
    } finally {
      setCopyingId(null);
    }
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <Card padded={false}>
        ...
        {copyError && (
          <div className="mx-5 mb-3 rounded-lg bg-error/10 px-4 py-3 text-sm font-medium text-error">{copyError}</div>
        )}
          ...
          <table className="w-full text-left min-w-[860px]">
            ...
            <tbody className="text-sm">
              {filtered.map((invoice) => {
                ...
                const canCopyLink = invoice.status !== "DRAFT" && invoice.status !== "CANCELLED";
                return (
                  <tr key={invoice.id} className="border-t border-surface-container-highest">
                    ...
                    <td className={`${tdClass} text-right`}>
                      <button
                        type="button"
                        onClick={() => handleCopyPayNowLink(invoice)}
                        disabled={!canCopyLink || copyingId === invoice.id}
                        className="p-1.5 text-on-surface-variant hover:text-primary disabled:opacity-40 disabled:hover:text-on-surface-variant"
                        title={canCopyLink ? "Copy Pay Now link" : "Send the invoice first to generate a Pay Now link"}
                      >
                        <Icon name={copiedId === invoice.id ? "check" : "content_copy"} className="text-[18px]" />
                      </button>
                    </td>
                  </tr>
                );
